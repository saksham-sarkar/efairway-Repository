...
import com.efairway.shop.model.catalog.product.PersistableImage;
...
public class PersistableProductPopulator extends
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(PersistableProductPopulator.class);
	...
	public Product populate(PersistableProduct source,
		...
		try {
			...
			if(!CollectionUtils.isEmpty(source.getDescriptions())) {
				for(com.efairway.shop.model.catalog.product.ProductDescription description : source.getDescriptions()) {
...
/*				if(productAvailability.getId() != null && productAvailability.getId().longValue() == 0) {
			...
			if(source.getAttributes()!=null) {
				for(PersistableProductAttribute attr : source.getAttributes()) {
			...
			if(!CollectionUtils.isEmpty(source.getCategories())) {
				for(com.efairway.shop.model.catalog.category.Category categ : source.getCategories()) {
